...
export default function AsambleaModal({
  open,
  asamblea,
  loading,
  onClose,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4">
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          <div>
            <h2 className="text-lg font-bold text-slate-900">
              {asamblea ? 'Editar multa' : 'Nueva multa'}
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Registra la reunión y los montos de asistencia.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-5 p-6 sm:grid-cols-2">
          ...
          <ElegantInput
            label="Multa por falta (Bs)"
            name="monto_multa"
            type="number"
            value={form.monto_multa}
            onChange={change}
            error={errors.monto_multa}
            required
          />
          <ElegantInput
            label="Multa por retraso (Bs)"
            name="monto_retraso"
            type="number"
            value={form.monto_retraso}
            onChange={change}
            error={errors.monto_retraso}
            required
          />
        </div>
